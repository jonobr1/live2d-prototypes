...
import { csmString } from '@live2d/framework/type/csmstring';
...
import { frameBuffer, AppDelegate } from './AppDelegate';
...
export class AppModel extends CubismUserModel {
  ...
  private setupModel(setting: ICubismModelSetting): void {
    ...
    if (this._modelSetting.getModelFileName() != '') {
      ...
      AppPal.printMessage('Model data does not exist.');
    }
  ...
  public update(): void {
    ...
    const deltaTimeSeconds: number = AppPal.getDeltaTime();
  ...
  public startMotion(
    ...
    if (priority == AppDefine.PriorityForce) {
      this._motionManager.setReservePriority(priority);
    } else if (!this._motionManager.reserveMotion(priority)) {
      if (this._debugMode) {
        AppPal.printMessage("[APP]can't start motion.");
      }
    ...
    if (this._debugMode) {
      AppPal.printMessage(`[APP]start motion: [${group}_${no}`);
    }
  ...
  public setExpression(expressionId: string): void {
    ...
    if (this._debugMode) {
      AppPal.printMessage(`[APP]expression: [${expressionId}]`);
    }
    ...
    if (motion != null) {
      this._expressionManager.startMotionPriority(
        motion,
        false,
        AppDefine.PriorityForce
      );
    } else {
      if (this._debugMode) {
        AppPal.printMessage(`[APP]expression[${expressionId}] is null`);
      }
    }
  }
  ...
  public motionEventFired(eventValue: csmString): void {
    CubismLogInfo('{0} is fired on AppModel!!', eventValue.s);
  }
  ...
  public constructor() {
    ...
    if (AppDefine.MOCConsistencyValidationEnable) {
