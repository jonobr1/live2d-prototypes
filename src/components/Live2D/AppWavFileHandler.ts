...
export let s_instance: AppWavFileHandler | null = null;

export class AppWavFileHandler {
  /**
   * クラスのインスタンス（シングルトン）を返す。
   * インスタンスが生成されていない場合は内部でインスタンスを生成する。
   *
   * @return クラスのインスタンス
   * @deprecated このクラスでのシングルトンパターンの使用は非推奨となりました。代わりに new AppWavFileHandler() を使用してください。
   */
  public static getInstance(): AppWavFileHandler {
    if (s_instance == null) {
      s_instance = new AppWavFileHandler();
    }
